import numpy as np
from LW_ArgMax_Updated import LW_ArgMax
from multiprocessing import Process , Queue
from utils import *

def compute_a_plus(q, params, arm_set, theta_warmup, estimate_theta, eta, best_arm, theta , seed):
    a_plus = LW_ArgMax(params, arm_set, theta_warmup , estimate_theta / np.linalg.norm(estimate_theta), eta, best_arm, theta , seed)
...
def compute_a_minus(q, params, arm_set, theta_warmup , estimate_theta, eta, best_arm, theta , seed):
    a_minus = LW_ArgMax(params, arm_set, theta_warmup , -estimate_theta / np.linalg.norm(estimate_theta), eta, best_arm, theta , seed)
...
def LWS(params , arm_set , theta_warmup ,eta , best_arm , theta , warmup_flag = False):
    """
    ...
    for i in range(params["dimension"]):
        ...
        if warmup_flag:
            ...
            q = Queue()
            # Create processes
            process_a_plus = Process(target=compute_a_plus, args=(q, params, arm_set, theta_warmup , estimate_theta, eta, best_arm, theta , ((i+1) ** (i+1) + i)))
            process_a_minus = Process(target=compute_a_minus, args=(q, params, arm_set, theta_warmup , estimate_theta, eta, best_arm, theta , ((i+1) ** (i+1) + 10 * (i+1))))
            # Start processes
    ...
    while replacement:
        ...
        for i in range(params["dimension"]):
            ...
            if warmup_flag:
                ...
                q = Queue()
                # Create processes
                process_a_plus = Process(target=compute_a_plus, args=(q, params, arm_set, theta_warmup ,estimate_theta, eta, best_arm, theta , ((i+2) ** (i+2) - i)))
                process_a_minus = Process(target=compute_a_minus, args=(q, params, arm_set, theta_warmup ,estimate_theta, eta, best_arm, theta , ((i+2) ** (i+2) + 9 * (i+1)**i + i)))
                # Start processes
                process_a_plus.start()
                process_a_minus.start()
                # Wait for both processes to complete
