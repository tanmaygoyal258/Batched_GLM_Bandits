...
import tqdm as tqdm
from utils import *
from Barycentric_Spanner_Updated import LWS

class SoftBatch_Updated:
    ...
    def play(self , time_round):
        if self.current_batch == 0:
            ...
            if self.arm_idx_being_played < len(self.final_barycentric_spanner):
                ...
                scaling_factor = min(np.sqrt(dsigmoid(np.dot(self.theta_warmup , arm))) , np.sqrt(dsigmoid(np.dot(self.theta_warmup , self.best_arm))))  
                upper_bound_on_arm_being_played = np.ceil(self.batch_lengths[self.current_batch] / (8 * self.dim * (1 + self.eta * scaling_factor * current_gap)**2))
                new = False
                if self.current_arm_pulls <= 1:
    ...
    def update_params(self , time_round , reward , arm_played):
        ...
        if time_round == self.batch_endpoints[0]:
            ...
            self.eta = 1 / (100 * self.gamma * self.dim * np.exp(4))
            self.best_arm = approximate_additive_oracle(self.arms , self.theta , 1/self.horizon , time_round)
            self.barycentric_spanner = LWS(self.params , self.arms + self.ball_spanning_vectors.tolist() , self.theta_warmup , self.eta , self.best_arm , self.theta , warmup_flag = False)
            ...
            # find the next barycentric spanner
            self.eta = self.batch_lengths[self.current_batch] / (100 * self.gamma * self.dim * np.exp(4))
            self.barycentric_spanner = LWS(self.params , self.arms + self.ball_spanning_vectors.tolist() , self.theta_warmup , self.eta , self.best_arm , self.theta , warmup_flag = False)
            self.final_barycentric_spanner = []
            for arm in self.barycentric_spanner:
                if arm in self.ball_spanning_vectors:
