...
class SoftBatch:
    ...
    def __init__(self , params , arms , kappa , epsilon):
        ...
        self.arms = arms
        self.scaled_arms = arms
        self.scale = 1
        self.scalings = {tuple(arm) : 1 for arm in self.scaled_arms}
        self.best_arm = self.arms[np.random.randint(len(self.arms))]
        
        self.ball_spanning_vectors = 1/self.horizon * np.identity(self.dim)
        # self.warmup_barycentric_spanner = LWS(params , self.arms + self.ball_spanning_vectors.tolist() , 0 , self.best_arm , self.theta , warmup_flag = True)
        ...
        self.best_arm = approximate_additive_oracle(self.scaled_arms , self.theta , 1/self.horizon , 0)
        if len(self.scaled_arms) <= self.dim:
            self.final_barycentric_spanner = self.scaled_arms
        else:
            self.barycentric_spanner = LWS(self.params , self.scaled_arms + self.ball_spanning_vectors.tolist() , self.scale , self.eta , self.best_arm , self.theta , warmup_flag = False)
    ...
    def play(self , time_round):
        ...
        if self.arm_idx_being_played < len(self.final_barycentric_spanner):
            scaled_arm = self.final_barycentric_spanner[self.arm_idx_being_played]
            current_gap = np.dot(self.best_arm , self.theta) - np.dot(scaled_arm , self.theta) if self.current_batch != 0 else self.param_norm_ub
            upper_bound_on_arm_being_played = np.ceil(self.batch_lengths[self.current_batch] * self.scale**2 / (8 * self.scalings[tuple(scaled_arm)]**2 * self.dim * (1 + self.eta * current_gap)**2))
            new = False
            ...
            if self.current_arm_pulls <= upper_bound_on_arm_being_played:
                ...
                return scaled_arm / self.scale , new
            else:
                ...
                if self.arm_idx_being_played == len(self.final_barycentric_spanner):
                    ...
                    return self.best_arm / self.scale , True
                return self.final_barycentric_spanner[self.arm_idx_being_played] / self.scale , new
        else:
            return self.best_arm / self.scale , False
    ...
    def update_params(self , time_round , reward , arm_played):
        ...
        # if time_round == self.batch_endpoints[0]:
            ...
            # self.scalings = {tuple(scaled_arm) : 1 for scaled_arm , arm in zip(self.scaled_arms , self.arms)}
            ...
            # self.eta = 1 / (8 * self.gamma * self.dim * np.exp(4))
            # self.best_arm = approximate_additive_oracle(self.scaled_arms , self.theta , 1/self.horizon , time_round)
            # if len(self.scaled_arms) <= self.dim:
            #     self.final_barycentric_spanner = self.scaled_arms
            # else:
            #     self.barycentric_spanner = LWS(self.params , self.scaled_arms + self.ball_spanning_vectors.tolist() , self.eta , self.best_arm , self.theta , warmup_flag = False)
            #     self.final_barycentric_spanner = []
            #     for arm in self.barycentric_spanner:
            #         if arm in self.ball_spanning_vectors:
            #             continue
